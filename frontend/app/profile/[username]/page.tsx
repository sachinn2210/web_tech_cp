  ...
  return (
    <div className="min-h-screen" style={{ background: '#f8f8f6' }}>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {/* Profile card — NO overflow-hidden on the outer card */}
        <div className="bg-white rounded-xl border-2 border-gray-200 mb-5">

          {/* Banner — overflow-hidden only here so rounded corners work */}
          <div className="h-24 sm:h-28 bg-gray-900 rounded-t-xl overflow-hidden relative">
            <div
          ...
          <div className="px-4 sm:px-6 pb-6">
            ...
            {/* Avatar + name row — avatar overlaps banner via negative margin */}
            <div className="flex flex-col sm:flex-row sm:items-end gap-3 sm:gap-4 -mt-10">

              {/* Avatar — z-10 so it sits on top of banner */}
              <div className="z-10 w-20 h-20 bg-gray-900 rounded-2xl flex items-center justify-center text-white font-black text-3xl ring-4 ring-white shadow-lg shrink-0">
                {profile.username.charAt(0).toUpperCase()}
              </div>

              {/* Name + rep */}
              <div className="flex flex-1 flex-col sm:flex-row sm:items-end sm:justify-between gap-3 pb-0 sm:pb-1 pt-2 sm:pt-10">
                <div>
                  <h1 className="text-xl sm:text-2xl font-black text-gray-900 leading-tight">{profile.username}</h1>
                  <p className="text-sm font-semibold text-gray-400">@{profile.username}</p>
                </div>

                {/* Reputation badge */}
                <div className="flex items-center gap-2 bg-gray-900 px-3 sm:px-4 py-2 rounded-xl self-start shrink-0">
              ...
              {editingBio ? (
                <div className="space-y-2">
                  <textarea
                    ...
                    rows={3}
                    className="w-full border-2 border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 focus:outline-none focus:border-gray-900 resize-none transition-colors"
                    placeholder="Tell us about yourself..."
                  />
                  <div className="flex gap-2">
                    <button onClick={handleSaveBio} className="px-4 py-1.5 bg-gray-900 text-white text-xs font-bold rounded-lg hover:bg-gray-700 transition-colors">
                      Save
                    </button>
                    <button
                ...
                <div>
                  {profile.bio ? (
                    ...
                    <p className="text-gray-400 text-sm italic">No bio yet. Add one to introduce yourself!</p>
                  ) : null}
                  {user?.username === username && (
                    <button
                      onClick={() => setEditingBio(true)}
                      className="mt-2 text-xs font-bold text-gray-500 hover:text-gray-900 border-2 border-gray-200 hover:border-gray-400 px-3 py-1 rounded-lg transition-all"
                    >
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-4 bg-white rounded-xl border-2 border-gray-200 p-1">
          ...
          <div className="space-y-3">
            {questions.length === 0 ? (
              <div className="bg-white rounded-xl border-2 border-dashed border-gray-300 p-12 text-center">
                <p className="text-gray-500 font-semibold text-sm">No questions yet</p>
              </div>
            ) : questions.map(q => (
              <Link key={q.id} href={`/question/${q.id}`} className="block">
                <div className="bg-white rounded-xl border-2 border-gray-200 hover:border-gray-900 p-4 transition-all group">
                  <h3 className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors mb-2 text-sm sm:text-base">{q.title}</h3>
                  <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-gray-400">
                    ...
                    <span className="text-gray-300">{new Date(q.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                  </div>
                </div>
              </Link>
            ))}
        ...
        {activeTab === 'answers' && (
          <div className="space-y-3">
            {answers.length === 0 ? (
              <div className="bg-white rounded-xl border-2 border-dashed border-gray-300 p-12 text-center">
                <p className="text-gray-500 font-semibold text-sm">No answers yet</p>
              </div>
            ) : answers.map(a => (
              <div key={a.id} className="bg-white rounded-xl border-2 border-gray-200 p-4">
                <Link href={`/question/${a.question.id}`} className="block">
                  <h3 className="font-bold text-gray-900 hover:text-blue-600 transition-colors mb-2 text-sm sm:text-base">{a.question.title}</h3>
                </Link>
                <p className="text-gray-600 text-sm line-clamp-2 mb-3 leading-relaxed">{a.content}</p>
